...
class EventsAdapter(private val events: List<Event?>, context: Context) : RecyclerView.Adapter<EventsAdapter.EventViewHolder>() {
    ...
    inner class EventViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(event: Event) {
            ...
            val placeNameText = if (!event.placeName.isNullOrEmpty()) " • ${event.placeName}" else ""
            eventInfo.text = "${event.dateTime!!.day!!} $monthName • ${event.dateTime!!.hour}:${event.dateTime!!.minute}$placeNameText"
